...
class DampedVibrationEDO:
    ...
    def approximate(self, t, h, x):
        return Vector(
            x[1], self.force_func(t) - (self.a**2) * x[0] - 2 * self.b * x[1]
        )
    ...
    def x(self, t):
        ...
        if self.discriminant > 0:
            ...
            mt_1 = m_1 * e ** (m_2 * t)
            mt_2 = m_2 * e ** (m_1 * t)
            return (x_0 / (m_1 - m_2)) * (mt_1 - mt_2)
        elif self.discriminant < 0:
            """ODamped Oscilation"""
            alpha = (-self.discriminant) ** (1 / 2)
            theta = atan(b / alpha)
            cos_t = cos(alpha * t - theta)
            delta = (alpha**2 + b**2) ** (1 / 2)
            return (x_0 * delta / alpha) * (e ** (-b * t)) * cos_t
        else:
            """Critically Dammped"""
...
if __name__ == "__main__":
    ...
    initial_condition = Vector(
        0,
        0,
    )  # Initial Charge  # Initial Current
    steps = 100000
    ...
    E_ax = cartesian_axes(fig.add_subplot(3, 1, 1), *axes_args)
    E_ax.set_title("Source")
    Q_max_cap = max((C * E_0, 1))
    Q_ax = cartesian_axes(
        fig.add_subplot(3, 1, 2), interval, Interval(-Q_max_cap, Q_max_cap)
    )
    Q_ax.set_title("Charge")
    I_max_ohm = max((1.2 * (E_0 / R), 1))
    I_ax = cartesian_axes(
        fig.add_subplot(3, 1, 3), interval, Interval(-I_max_ohm, I_max_ohm)
    )

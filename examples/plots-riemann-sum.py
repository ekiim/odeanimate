...
def riemann_sums_plot(f, interval, n, filenumber=None, name=""):
    ...
    eq_axes.text(0, 0.5,
         s="interval:".capitalize(),
        fontsize=fontsize
    )
    eq_axes.text(0.5, 0.5,
         s="{}".format(interval.limits).capitalize(),
        fontsize=fontsize
    )
    eq_axes.text(0, 0.4,
         s="partition:".capitalize(),
        fontsize=fontsize
    )
    eq_axes.text(0.5, 0.4,
         s="{}".format(n),
        fontsize=fontsize
    )
    sum_text_start = 0.3
    ...
    for (i, ax, variant) in zip(range(3), rs_axes, ('mid', 'left', 'right')):
        ...
        eq_axes.text(
            0, sum_text_start - i*0.1,
            s="{}".format(variant).capitalize(),
            fontsize=fontsize
        )
        eq_axes.text(
            0.5, sum_text_start - i*0.1,
            s="{:02.5}".format(area).capitalize(),
            fontsize=fontsize
        )
    image_file = output_file(__file__, '-{}-{:04}.png'.format(
        name,
        filenumber if filenumber else n
    ))

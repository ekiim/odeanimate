...
def nth_roots_of_unity(n=1):
    return tuple(
        complex(
            tolerance_or_zero(cos(2*pi*k/n)), tolerance_or_zero(sin(2*pi*k/n))
        )
...
def output_file(file, ext, project_root=Path(__file__).parent.parent, doc=None, **kwargs):
...
class Interpolator:
    ...
    def __call__(self, t):
        ...
        lower = min(
            filter(lambda k: k < self.values.keys()),
            key=lambda k: abs(t - k)
        )
        upper = min(
            filter(lambda k: k > self.values.keys()),
            key=lambda k: abs(t - k)
